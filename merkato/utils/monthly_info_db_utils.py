...
def create_monthly_info_table():
    ...
    try:
        ...
        c.execute('''CREATE TABLE IF NOT EXISTS monthly_info
                    ...
                    mm_base_profit float, mm_quote_profit float, starting_usd_val float, ending_usd_val float, last_price float)''')
        conn.commit()
        conn.close()
...
def insert_monthly_info(exchange_pair='tuxBTC_ETH', spread='.1', last_price=.018, step=1.0033, start_base=0, start_quote=0, 
    end_base=0, end_quote=0, mm_base_profit=0, mm_quote_profit=0, starting_usd_val=0, ending_usd_val=0):
    ''' TODO: Function Comment
    '''
    try:
        conn = sqlite3.connect('merkato.db')
        ...
        c.execute("""INSERT INTO monthly_info 
                    (exchange_pair, spread, step, start_base, start_quote, end_base, end_quote, mm_base_profit, mm_quote_profit, starting_usd_val, ending_usd_val, last_price) VALUES (?,?,?,?,?,?,?,?,?,?,?,?)""",
                    (exchange_pair, spread, step, start_base, start_quote, end_base, end_quote, mm_base_profit, mm_quote_profit, starting_usd_val, ending_usd_val, last_price))
        conn.commit()
        conn.close()
